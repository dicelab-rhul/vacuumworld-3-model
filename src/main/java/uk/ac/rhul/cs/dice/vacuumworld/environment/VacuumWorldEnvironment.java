...
public class VacuumWorldEnvironment extends AbstractEnvironment implements Runnable {
	...
	private void printActorDetails(VacuumWorldAbstractActionInterface action, String moment) {
		...
		LogUtils.log(actor.getID()
				...
				+ moment
				+ " attempt: "
				+ this.grid.entrySet().stream().filter(e -> e.getValue().containsSuchActor(actor.getID())).findFirst().map(Entry::getKey)
						.orElse(null) + ".");
	...
	public VacuumWorldActor getActorFromId(String id) {
		...
		return this.grid.values().stream().map(VacuumWorldLocation::getActorIfAny).filter(Objects::nonNull).filter(actor -> id.equals(actor.getID()))
