...
public class VacuumWorldEnvironmentAppearance implements EnvironmentAppearance, VacuumWorldGridPerceptionInterface {
	...
	/**
	 * 
	 * Constructs a {@link VacuumWorldEnvironmentAppearance} from a {@link Map} from {@link VacuumWorldCoordinates} to
	 * {@link VacuumWorldLocation} representing the grid.
	 * 
	 * @param grid
	 *            a {@link Map} from {@link VacuumWorldCoordinates} to {@link VacuumWorldLocation} representing the
	 *            grid.
	 * 
	 */
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithCleaningAgents() {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isACleaningAgentThere)
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithOrangeAgents() {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isAnOrangeAgentThere)
				.collect(Collectors.toSet()));
	}
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithCleaningAgentsExcludingSelf(String selfId) {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isACleaningAgentThere)
				.filter(loc -> !loc.getAgentAppearanceIfAny().getId().equals(selfId)).collect(Collectors.toSet()));
	}

	@Override
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithGreenAgentsExcludingSelf(String selfId) {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isAGreenAgentThere)
				.filter(loc -> !loc.getAgentAppearanceIfAny().getId().equals(selfId)).collect(Collectors.toSet()));
	}
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithWhiteAgentsExcludingSelf(String selfId) {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isAWhiteAgentThere)
				.filter(loc -> !loc.getAgentAppearanceIfAny().getId().equals(selfId)).collect(Collectors.toSet()));
	}
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsWithActiveBodies() {
		return ImmutableSet
				.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isAnActiveBodyThere).collect(Collectors.toSet()));
	}
	...
	public Set<VacuumWorldLocationAppearance> getAllLocationsFreeFromActiveBodies() {
		return ImmutableSet.copyOf(this.grid.values().stream().filter(VacuumWorldLocationAppearance::isFreeFromActiveBodies)
				.collect(Collectors.toSet()));
	}
